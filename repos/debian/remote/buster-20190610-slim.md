## `debian:buster-20190610-slim`

```console
$ docker pull debian@sha256:bcde50aa681092b7a5cf059d1e76cd87e7d0b0f5656712345c31b0d7e7f3c71b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms:
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; ppc64le
	-	linux; s390x

### `debian:buster-20190610-slim` - linux; amd64

```console
$ docker pull debian@sha256:0fd2c346de4fb5be7faead089c76527a7305393921b419fa9d197ffd43a713e3
```

-	Docker Version: 18.06.1-ce
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.1 MB (27093818 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2dae943fc808e77188bccde0e764156d1a6f1d06d4f2af25fe221a716ab30199`
-	Default Command: `["bash"]`

```dockerfile
# Mon, 10 Jun 2019 23:21:54 GMT
ADD file:db9367210dc13ab3e6857a0ee87a882acfc6bb633d10ca8297527088fcb7c1a6 in / 
# Mon, 10 Jun 2019 23:21:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:782229fa27bead2dc1212df54174729d244d7e01fa0eba568d156d5579a7c592`  
		Last Modified: Mon, 10 Jun 2019 23:26:04 GMT  
		Size: 27.1 MB (27093818 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20190610-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:21d0772ac870f4c686538a1f9d3a2a57c6542315c867d51a1931cdac1c3fcd4a
```

-	Docker Version: 18.06.1-ce
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.8 MB (24820842 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db6402fdf739868b08f8717aa57275ce9aa6cec98bdf36f6c21e71c248fc2135`
-	Default Command: `["bash"]`

```dockerfile
# Mon, 10 Jun 2019 23:49:27 GMT
ADD file:f25a46605412d2e4ce5efc343f5784417289ce4954b46377b9bdb8e7ccbd04bd in / 
# Mon, 10 Jun 2019 23:49:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b18a35cb2df964af86b6d6c0e5443d304d0ac35af310cfbb2c00ea17ca31bb95`  
		Last Modified: Mon, 10 Jun 2019 23:56:25 GMT  
		Size: 24.8 MB (24820842 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20190610-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:c79af5085a3ef9472bf41d1c115e40737781ca38ec18435115f28421c5f9bb6f
```

-	Docker Version: 18.06.1-ce
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.7 MB (22697729 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40924754353b33df7d27ded09b0f3c29d61596f321657ac1996eeeecdc2b96a7`
-	Default Command: `["bash"]`

```dockerfile
# Mon, 10 Jun 2019 23:58:18 GMT
ADD file:596f85197b2dd6b39ec3f1de4832c26353705986aac8b27ea47bf4370b52c876 in / 
# Mon, 10 Jun 2019 23:58:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:57c36a17ef502075ceabddac9f6c9f92a5e9594c830f6bad2edd87b937dd5532`  
		Last Modified: Tue, 11 Jun 2019 00:06:16 GMT  
		Size: 22.7 MB (22697729 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20190610-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:29d6e09640fda31acf646ae34643d37638f2de94a9c28d67110fbdabd7f542e4
```

-	Docker Version: 18.06.1-ce
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25852408 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c23538595cb586a89c4fbffd1dafbe82868993d8cdbbb5ba230eed7ce1b818c9`
-	Default Command: `["bash"]`

```dockerfile
# Mon, 10 Jun 2019 23:40:22 GMT
ADD file:8da39d68d99c4003ff960332f4402d553836705d14c35b9ff2ccf74a3c20d90b in / 
# Mon, 10 Jun 2019 23:40:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:83b836c5003339192041fc4655acc9a536d303dc07ba86f5fb1694b79a0306aa`  
		Last Modified: Mon, 10 Jun 2019 23:43:36 GMT  
		Size: 25.9 MB (25852408 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20190610-slim` - linux; 386

```console
$ docker pull debian@sha256:a7c1027b05b32878569aabcbe646932b8012ced51d4f0813bea07ff522d22420
```

-	Docker Version: 18.06.1-ce
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.7 MB (27745534 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7ffe5fa473f31680e58d383735ec746acd10198585cde738dd414027a0007270`
-	Default Command: `["bash"]`

```dockerfile
# Mon, 10 Jun 2019 23:39:15 GMT
ADD file:36fcfcac7934ae2714a2f7d4d0bbb4eae7f9e928dbc5a5daad09d92d490a0ce4 in / 
# Mon, 10 Jun 2019 23:39:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:40009893fe9b091ac0d66b392e309c3e06d2edd3c6e0506ab5cbe8efa5a4a290`  
		Last Modified: Mon, 10 Jun 2019 23:43:20 GMT  
		Size: 27.7 MB (27745534 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20190610-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:7b0638e9509614efdcf9b302001521aa3755e404898b98addd17c90db682354c
```

-	Docker Version: 18.06.1-ce
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.5 MB (30514426 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8acc70e95e63f8305e4c3d2d386364596e98cfca9c0210beab6e3eb2450ca2d3`
-	Default Command: `["bash"]`

```dockerfile
# Mon, 10 Jun 2019 23:17:29 GMT
ADD file:102525da73fab3db4cb6f21e1dc74b75c2ed1114550502e826e92c530e3b2341 in / 
# Mon, 10 Jun 2019 23:17:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a31ab3e9dea04f788e7ea80c091ebd2ad6e8a4207d0617896ca4485f5903834a`  
		Last Modified: Mon, 10 Jun 2019 23:21:46 GMT  
		Size: 30.5 MB (30514426 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20190610-slim` - linux; s390x

```console
$ docker pull debian@sha256:fbf96c555b597d6c9ad71b6b12635bdba77a57471ef8a02feb278b0c887361c3
```

-	Docker Version: 18.06.1-ce
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.7 MB (25703472 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0f16d9de4fdde96553d0b8fcb1288f14fa3f22888662e7e83351019491832820`
-	Default Command: `["bash"]`

```dockerfile
# Mon, 10 Jun 2019 23:42:28 GMT
ADD file:f61c0ff96b9ab1049d755ffe09cfec7609fe8340229b60d393c62ddb39e3d597 in / 
# Mon, 10 Jun 2019 23:42:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:846ee1802bfd7e14560d51ebed97afa8ec6f685f0ba33507d1f347ad2ef1f1ff`  
		Last Modified: Mon, 10 Jun 2019 23:45:29 GMT  
		Size: 25.7 MB (25703472 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
